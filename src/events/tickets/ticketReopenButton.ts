...
export default async function ticketReopenButton(interaction: ButtonInteraction): Promise<void> {
  ...
  await interaction.channel.permissionOverwrites.edit(ticket.creatorId, { SendMessages: true, ViewChannel: true });
  ...
  await interaction.channel.send({ content: `🔓 Reopened by ${interaction.user}.` });
}